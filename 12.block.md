...
```
联想以下C结构的初始化方法，实际上是结构体__main_block_impl_0通过其构造函数进行了初始化，其构造函数带有两个参数。这个__main_block_impl_0结构体就是我们的block对象。如下我们看到，block对象主要包含了一个__block_impl结构体和一个__main_block_desc_结构体，当然还有隐式的isa指针。
```
//block实现的结构体，block初始化入口
struct __main_block_impl_0 {
  //block结构体实例
